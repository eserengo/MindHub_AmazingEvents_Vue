...
const App = createApp({
  ...
  computed: {
    ...
    filteredData() {
      return this.upcomingEventsArray.filter(item => this.filterInput.find(subitem => subitem == item.category) !== undefined);
    },

    searchedData() {
      if (!this.filterInput.length) {
        return this.upcomingEventsArray.filter(item => {
    ...
    categories() {
      return this.upcomingEventsArray && Array.from(new Set(this.upcomingEventsArray.map(item => item.category)));
    },
  },
  ...
  <header class="p-2">
          ...
          <div class="col-2 col-sm-8 col-lg-6">
            ...
            <div class="collapse navbar-collapse w-100 justify-content-sm-end" id="navbar">
              <ul
                ...
                <li class="nav-item">
                  <a href="../pages/upcomingEvents.html" aria-current="page"
                    class="nav-link active fs-6" style="color: rgb(214, 0, 100);">Upcoming Events</a>
                </li>
                <li class="nav-item">
                  <a href="../pages/pastEvents.html" class="nav-link fs-6">Past Events</a>
                </li>
                <li class="nav-item">
    ...
    <section id="carousel" class="carousel slide position-relative">
      ...
      <h1 class="position-absolute top-50 start-50 translate-middle text-white" style="font-size: 64px;">Upcoming Events</h1>
    </section>
  </header>
  ...
  <main>
    ...
    <div v-if="this.fetchedData.events" class="container-fluid p-2">
      ...
      <div v-if="!this.filterInput.length && !this.searchInput.length"
        class="row w-100" v-html="this.createEventsCards(upcomingEventsArray)"></div>
